...
type t = {
  ...
  mutable rtt_timer_starttime: int;
  mutable srtt: int;
  mutable rttvar: int;
  mutable rto: int;
...
let alpha = 125000  (* see RFC 2988 *)
let beta = 250000    (* see RFC 2988 *)
  ...
  if t.fast_recovery then begin
	...
	if t.rtt_timer_reset then begin
	  t.rtt_timer_reset <- false;
          t.rttvar <- (500000 * rtt_m);
          t.srtt <- rtt_m;
	end else begin
          t.rttvar <- (((1000000 - beta) * t.rttvar) + (beta * (abs (t.srtt - rtt_m))));
          t.srtt <- (((1000000 - alpha) * t.srtt) + (alpha * rtt_m));
	end;
        t.rto <- (max 1000000 (t.srtt + (4000000 * t.rttvar)));
      end;
    end;
