...
CAMLprim value caml_bigarray_base_offset(value v_ba);
CAMLprim value caml_bigarray_shift_left(value v_ba, value v_len);
...
caml_bigarray_base_offset(value v_ba)
{
  ...
  if (ba->proxy == NULL)
    ...
    off_t len = (((char *) ba->data) - ((char *) ba->proxy->data));
