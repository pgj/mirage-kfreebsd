...
CAMLprim value caml_output_value_to_buffer(value buf, value ofs, value len, value v, value flags);
...
static void extern_record_location(value obj)
{
  ...
  if (extern_trail_cur == extern_trail_limit) {
    struct trail_block * new_block = __malloc(sizeof(struct trail_block));
...
static void init_extern_output(void)
{
  ...
  extern_output_first = __malloc(sizeof(struct output_block));
...
static void grow_extern_output(intnat required)
{
  ...
  if (required <= SIZE_EXTERN_OUTPUT_BLOCK / 2)
    ...
    extra = required;
  blk = __malloc(sizeof(struct output_block) + extra);
